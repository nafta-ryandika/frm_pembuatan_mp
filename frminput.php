    ...
    <table width="100%"  border="0" cellspacing="0" cellpadding="0">
      ...
      <tr>
        <td>
          ...
          <input id="inartprod" class="textbox" type="text" name="intype" value="" maxlength="10" style="width: 250px">
        </td>
      </tr>
      <tr>
        <td>
          <label>Art. Customer</label>
          <input id="inartcust" class="textbox" type="text" name="intype" value="" maxlength="15" style="width: 250px">
        </td>
      </tr>
      <tr>
        <td>
      ...
      <tr>
        <td>
          <label>SKU</label>
          <input id="insku" class="textbox" type="text" name="intype" value="" maxlength="60" style="width: 250px" readonly="">
  ...
  <legend>Detail MP
          ...
          while ($data = mysql_fetch_array($result)) {
            ...
            if ($innompx != "") {
              $sql_1 = "SELECT 
                         ...
                         a.nosubpkj
                        FROM clmpdet2 a
                        LEFT JOIN kmmstsubpkj b ON a.subpkj = b.subpkj
                        LEFT JOIN kmmstbhnbaku c ON a.materi = c.kdbrg
                        LEFT JOIN kmmstsupp d ON a.sup = d.kdsupp
            ...
            if ($count_1 > 0) {
              ...
              $sql_3 = "select max(a.nosubpkj) as maxnosubpkj from clmpdet2 a where a.mpno = '".$innompx."' and a.nopkj = '".$no."'";
              $result_3 = mysql_query($sql_3,$conn);
              $data_3 = mysql_fetch_array($result_3);
              $nosubpkjx = $data_3["maxnosubpkj"];
            }
        ...
        ?>
          <tr>
            ...
            <td colspan="10" style="text-align: left;" ><?=$data["ket"]?>
              ...
              <input type="hidden" id="row_subpkj_<?=$no?>" value="<?=(1+$nosubpkjx)?>" size="3">
        ...
        <?php
            if ($count_1 > 0) {
              ...
              while ($data_1 = mysql_fetch_array($result_1)) {
                $nosubpkj = trim($data_1["nosubpkj"]);
                $id_component = trim($data_1["subpkj"]);
                $component = trim($data_1["ket"]);
                $id_material = trim($data_1["kdbrg"]);
                $material = trim($data_1["nmbrg"]);
                $id_supplier = trim($data_1["kdsupp"]);
                $supplier = trim($data_1["nmsupp"]);
                $calc = trim($data_1["calc"]);
                $qty = trim($data_1["qty"]);
                $nstn = trim($data_1["nstn"]);
                $ambil = trim($data_1["ambil"]);
                $backorder = trim($data_1["backorder"]);

                echo "<tr>";
                  echo "<td style=\"text-align: center;\">";
                    echo "<input id='subpkj_".$no."_".$no_1."' class='subpkj_".$no."' type='hidden' style='width: 50px' value='".$nosubpkj."'>";
    ...
    <table id="myTable" class="table" width="100%">
      ...
      <tbody>
        <tr>
          <td valign="top">
            <textarea id="inrspecial" name="inrspecial" class="resize" onkeypress="" style="width:99%; min-height:150px;"></textarea>
          </td>
          <td valign="top">
            <textarea id="inrket" name="inrket" class="resize" onkeypress="" style="width:99%; min-height:150px;"></textarea>
          </td>
        </tr>
      </tbody>
    </table>
